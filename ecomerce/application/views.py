...
class PasswordChangeDoneView(TemplateView):
    template_name = 'application/passwordchangedone.html'

class MyPasswordChangeView(PasswordChangeView):
    template_name = 'application/changepassword.html'
...
class ProductDetail(View):
    def get(self,request, pk):
        ...
        if request.user.is_authenticated:
            ...
            wishitem = len(Wishlist.objects.filter(user=request.user))
        return render(request, "application/productdetail.html", {'product': product, 'wishlist_items': wishlist_items, 'totalitem': totalitem})
...
class CustomerRegistrationView(View):
    def get(self, request):
        ...
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
            wishitem = len(Wishlist.objects.filter(user=request.user))
        return render(request, 'application/customerregistration.html', {'form': form})
